...
function getCategoryPlaceholder(category){
  ...
  if(c.includes('terno')) return "https://images.unsplash.com/photo-1593032465175-481ac7f401a0?auto=format&fit=crop&w=1400&q=80";
  if(c.includes('linho')) return "https://images.unsplash.com/photo-1520975599570-5db0fe395ea0?auto=format&fit=crop&w=1400&q=80";
  if(c.includes('joia')) return "https://images.unsplash.com/photo-1524592094714-0f0654e20314?auto=format&fit=crop&w=1400&q=80";
  if(c.includes('perf')) return "https://images.unsplash.com/photo-1523292562811-8fa7962a78c8?auto=format&fit=crop&w=1400&q=80";
  if(c.includes('blazer')) return "https://images.unsplash.com/photo-1520975682071-aacbc3f4a78a?auto=format&fit=crop&w=1400&q=80";
  return "https://images.unsplash.com/photo-1520975682071-aacbc3f4a78a?auto=format&fit=crop&w=1400&q=80";
}
...
function normalizeProducts(list){
  ...
  return (list || []).map((p, idx)=>({
    ...
    image: String(p.image || p.images?.[0] || getCategoryPlaceholder(p.category) || "assets/placeholders/product.jpg"),
  ...
  return `
    <article class="card">
      <a class="card__img" href="product.html?id=${encodeURIComponent(p.id)}" aria-label="Ver ${p.name}">
        ${badgeHtml}
        <img src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.onerror=null;this.src='assets/placeholders/product.jpg';">
      </a>
