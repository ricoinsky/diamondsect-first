import { moneyBRL, getCart, saveCart, updateCartCount, findProduct, getProducts, getSession, completePurchase } from "./store.js";
...
function initMobileBarVisibility(){
  ...
  if(!document.body.classList.contains("cart-page")){
    ...
    document.body.classList.remove("ds-mobilebar-on");
    return;
  }
  ...
  function update(){
    ...
    const show = (y > 220 && !summaryVisible);
    bar.classList.toggle("is-visible", show);
    // classe usada para evitar sobreposição no mobile (chat, footer, safe-area)
    document.body.classList.toggle("ds-mobilebar-on", show);
  }
...
function renderCart(){
  ...
  if(!(catalog && catalog.length)){
    cartList.innerHTML = `
      <div class="empty">
        <h2>Seu carrinho está pronto ✨</h2>
        <p>Nenhum produto disponível no momento. Assim que as peças entrarem, você consegue finalizar por aqui.</p>
...
function initActions(){
  ...
  function calcTotals(){
    ...
    const subtotal = (cart||[]).reduce((sum,it)=>{
      const p = list.find(x=> Number(x.id)===Number(it.id));
      return sum + (Number(p?.price||0) * Number(it.qty||0));
    },0);

    const coupon = getCoupon();
    let discount = 0;
    if(coupon?.code === "DIAMOND10") discount = subtotal * 0.10;
    else if(coupon?.code === "DIAMOND15") discount = subtotal * 0.15;
    else if(coupon?.code === "VIP200") discount = Math.min(200, subtotal * 0.25);

    const ship = getShip();
    const shipping = Number(ship?.value || 0);
    const total = Math.max(0, subtotal - discount + shipping);
    return { subtotal, discount, shipping, total, coupon, ship };
  }

  function openDoneModal(orderId, total){
    const existing = document.getElementById("dsDoneModal");
    ...
    wrap.innerHTML = `
      <div class="dsModal__bg" data-close="1"></div>
      <div class="dsModal__card" role="dialog" aria-modal="true">
        <div class="dsModal__title">Compra confirmada ✅</div>
        <div class="dsModal__sub">Seu pedido foi registrado.</div>

        <div class="dsModal__box">
          <div class="dsModal__row"><span>Código do pedido</span><b>${orderId}</b></div>
          <div class="dsModal__row"><span>Total</span><b>${moneyBRL(total)}</b></div>
        </div>

        <div class="dsModal__hint">
          Agora você já pode voltar no produto e deixar sua avaliação (somente para itens comprados).
        </div>

        <div class="dsModal__actions">
          <a class="btn" href="index.html">Voltar para a loja</a>
  ...
  function finish(){
    const s = getSession();
    if(!s?.email){
      toast("Para finalizar e liberar avaliações, entre na sua conta.");
      if(window.DS_OPEN_AUTH) window.DS_OPEN_AUTH("login");
      return;
    }

    const totals = calcTotals();
    const res = completePurchase({ shipping: totals.shipping, discount: totals.discount, total: totals.total });
    if(!res.ok){
      toast(res.message || "Não foi possível finalizar.");
      if(res.code === "NO_SESSION" && window.DS_OPEN_AUTH) window.DS_OPEN_AUTH("login");
      renderCart();
      return;
    }

    // limpa cupom e frete do carrinho (opcional)
    setCoupon(null);
    setShip(null);

    renderCart();
    openDoneModal(res.orderId, totals.total);
  }
