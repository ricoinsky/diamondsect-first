import { findProduct, moneyBRL, addToCart, updateCartCount, getProducts, getSession, hasPurchased } from "./store.js";
...
function getFallbackImage(p){
  const cat = String(p?.category||"premium").toUpperCase();
  return placeholderSVG(p?.name || "Diamondsect", cat);
}
...
function render(){
  ...
  if(!images.length) images.push(getFallbackImage(p));
...
function initReviews(p){
  ...
  const s = getSession();
  const canReview = !!(s?.email && hasPurchased(p.id));

  // prefill name
  const nameInput = qs("#rvName");
  if(nameInput && s?.name) nameInput.value = s.name;

  if(!canReview){
    form.classList.add("is-locked");
    form.querySelectorAll("input,textarea,button").forEach(el=> el.disabled = true);
    const hintEl = qs("#rvHint");
    if(hintEl){
      hintEl.textContent = "Somente clientes que compraram este item podem avaliar. Finalize a compra logado para liberar.";
    }
    // botão entrar
    if(!s?.email){
      ...
      b.textContent = "Entrar para finalizar e avaliar";
  ...
  form.addEventListener("submit", (e)=>{
    ...
    const s = getSession();
    addReview(p.id, { id: `rv_${Date.now()}_${Math.random().toString(16).slice(2,8)}`, name, email: s?.email || "", rating:picked, comment:text, date: new Date().toISOString() });
    qs("#rvText").value = "";
